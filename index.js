...
// Create sessions table if not exists - with error handling
  ...
  const saveCreds = async () => {
    try {
      ...
      await pool.query(
        ...
        [sessionId, currentState.creds, currentState.keys]
      )
    } catch (err) {
...
async function startSock() {
  try {
    ...
    setTimeout(startSock, 5000) // Retry after 5 seconds
  }
}
  ...
  try {
    ...
    if (usePairCode) {
      if (!pairPhoneNumber || !/^\d{10,15}$/.test(pairPhoneNumber)) {
        console.error('Invalid phone number. Must be 10-15 digits. Example: 994400540665')
        process.exit(1)
      }
      console.log(`Starting in PAIR mode with number: ${pairPhoneNumber}`)
    } else {
      console.log('Starting in QR code mode')
    }
